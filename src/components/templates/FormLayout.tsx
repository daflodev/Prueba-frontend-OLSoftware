...
export const FormLayout = ({
    ...
    return (
        <div className="flex flex-col h-full bg-[#f4f6fc] relative">

            <div className="w-full bg-[#f4f6fc] px-10 py-4 border-b border-gray-200 shadow-sm">
                <h1 className="text-[20px] font-bold text-[#0a125a] tracking-tight">
                    {title || "Empresa 1"}
                </h1>
            </div>

            <div className="flex-1 overflow-y-auto px-6 py-10 pb-36">
                <div className="max-w-6xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 p-10">
                    {children}
                </div>
            </div>

            <footer className="absolute bottom-0 left-0 right-0 bg-[#2d3a8d] text-white h-24 flex items-center px-12 z-50">
                ...
                <div className="flex-1 flex items-center justify-end gap-10 pl-10">
                    <p className="text-sm font-medium text-blue-50 opacity-90 hidden lg:block">
                        Si ya ingresaste todos los datos, crea tu formulario aquí
                    </p>
                    <button
                        onClick={onSave}
                        disabled={isSubmitting}
                        className="bg-[#e81f76] hover:bg-[#c81965] text-white px-12 py-4 rounded-xl font-bold text-sm uppercase tracking-widest transition-all shadow-xl active:scale-95 disabled:opacity-50"
                    >
